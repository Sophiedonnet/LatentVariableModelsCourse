# Test genotype mixture for Taita thrush data (GLS00)
...
m <- sapply(1:p, function(j){length(unique(haplotypes[, j], na.rm=TRUE))})
haploArray <- MakeGenoArray(haplotypes)
F_Matrix2Tabular(cbind(id[1:6], haplotypes[1:6, ]))
...
kMax <- 5; tryNb <- 20
resName <- paste0(dataName, '-mixture-k', kMax, '-m')
