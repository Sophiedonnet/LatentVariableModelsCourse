...
pi <- switch(type_graph,
  ...
  save(pi,alpha,sbm,file=paste('plots_sbm/Data_',type_graph,'.Rdata',sep=''))
  ...
  file_save = paste('plots_sbm/',type_graph,'_graphe_without_colors.png',sep='')
  dev.copy(png,file_save)
  dev.off()
  
  
  plot(R,layout=layout,vertex.color=sbm$cl,vertex.size=7,vertex.label=NA)
  file_save = paste('plots_sbm/',type_graph,'_graphe_with_colors.png',sep='')
  dev.copy(png,file_save)
  dev.off()
  
  betw=betweenness(R)
  boxplot(betw~sbm$cl,main=paste('Betweeness by block'))
  file_save = paste('plots_sbm/',type_graph,'_betweeness.png',sep='')
  dev.copy(png,file_save)
  dev.off()
  
  deg= degree(R)
  hist(deg,main="Histogram of degrees")
  file_save = paste('plots_sbm/',type_graph,'_histogram_degree.png',sep='')
  dev.copy(png,file_save)
  dev.off()
  ...
  file_save = paste('plots_sbm/',type_graph,'_graphe_resume.png',sep='')
  dev.copy(png,file_save)
  dev.off()
  ...
  g
  file_save = paste('plots_sbm/',type_graph,'_adja.png',sep='')
  ...
  g
  file_save = paste('plots_sbm/',type_graph,'_reordered_adja_with_groups.png',sep='')
  ggsave(file_save, width = 20, height = 20, units = "cm")




  g2<- ggplot(data = melted_Adj_ord, aes(x=index_plants, y=index_animals, fill=link)) + geom_tile() +ggtitle("Reordered adjacency matrix")
  ...
  g2
  file_save = paste('plots_sbm/',type_graph,'_reordered_adja_without_groups.png',sep='')
  ggsave(file_save, width = 20, height = 20, units = "cm")
